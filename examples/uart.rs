...
struct UartCrc<'a> {
    serial: &'a mut Serial<USART1>,
}

impl<'a> UartCrc<'a> {
    pub fn new(serial: &'a mut Serial<USART1>) -> Self {
        UartCrc { serial }
    }
    ...
    pub fn listen_for_response(&mut self) -> Flags {
        let buf = block!(self.serial.read()).unwrap();
        let flags = Flags {
    ...
    pub fn send_chunk(&mut self, chunk: &Chunk) {
        ...
        for i in 0..CHUNK_SIZE {
            block!(self.serial.write(buf[i])).unwrap();
        }
    }
    ...
    pub fn listen_for_chunk(&mut self) -> Chunk {
        let mut buf: [u8; CHUNK_SIZE] = [0; CHUNK_SIZE];
        for i in 0..CHUNK_SIZE {
            buf[i] = block!(self.serial.read()).unwrap();
        }
